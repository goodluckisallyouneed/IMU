...
class InfluenceWeightedDataset(Dataset):
...
def get_last_params(model):
    return [p for n, p in model.named_parameters() if 'output_layer' in n or 'decoder' in n]
...
def compute_fisher_matrix(model, dataset, device, state_size):
    fisher_diag = None
    for data in tqdm(dataset, desc="Compute Fisher"):
        x = data['input_ids'].unsqueeze(0).to(device)
        t = data['labels'].unsqueeze(0).to(device)
        grad_list = grad_z_last(model, x, t, state_size)
        ...
        if fisher_diag is None:
            ...
            fisher_diag += squared_grad
    fisher_diag /= len(dataset)
    fisher_diag += 0.01
    return 1.0 / fisher_diag

def compute_all_gradients(model, dataset, device, state_size):
    grads = []
    grad_d2_total = None
    for data in tqdm(dataset, desc="Compute gradients"):
        x = data['input_ids'].unsqueeze(0).to(device)
        t = data['labels'].unsqueeze(0).to(device)
        grad_list = grad_z_last(model, x, t, state_size)
        flat_grad = flatten_grad(grad_list)
...
def compute_influence_weights(model, inv_fisher_diag, dataset, device, state_size):
    G, grad_d2_total = compute_all_gradients(model, dataset, device, state_size)
    weighted = G * inv_fisher_diag.unsqueeze(0)
    influence = -(weighted @ grad_d2_total)
    return influence.cpu()

def select_top_negative_influence(dataset, influences, top_percent):
    influences = np.array(influences)
    negative_mask = influences < 0
    negative_indices = np.where(negative_mask)[0]
    negative_influences = influences[negative_mask]
    sorted_indices = np.argsort(np.abs(negative_influences))[::-1]
    top_k = max(1, int(len(negative_influences) * top_percent))
    selected_indices = negative_indices[sorted_indices[:top_k]]
    weights = torch.sqrt(torch.tensor(np.abs(negative_influences[sorted_indices[:top_k]]), dtype=torch.float32))
    max_clip = torch.quantile(weights, 0.93)
    weights = torch.clamp(weights, max=max_clip)
    return Subset(dataset, selected_indices), weights

def l1_regularization(model):
    params_vec = []
    for name, param in model.named_parameters():
        if "output_layer" in name or "decoder" in name:
            params_vec.append(param.view(-1))
    return torch.linalg.norm(torch.cat(params_vec), ord=1)

def compute_combined_loss(model, batch, criterion, args, device):
    model.train()
    criterion = nn.CrossEntropyLoss(ignore_index=-100, reduction='none')
    for name, param in model.named_parameters():
        param.requires_grad = ('output_layer' in name or 'decoder' in name)
    ...
    influence_loss = - torch.dot(weights, loss_vector) / weights.sum()
...
def main(args):
    ...
    inv_fisher_diag = compute_fisher_matrix(model, forget_train_dataset, device, state_size)

    influences = compute_influence_weights(model, inv_fisher_diag, forget_train_dataset, device, state_size)

    top_percent = args.top_data 
    selected_dataset, weights = select_top_negative_influence(forget_train_dataset, influences, top_percent)

    weighted_dataset = InfluenceWeightedDataset(selected_dataset, weights)
    forget_train_dataloader = DataLoader(weighted_dataset, batch_size=batch_size, shuffle=True)

    print(f"[IMU] forget dataloader rebuilt with {len(weighted_dataset)} influential samples.")
    ...
    for epoch in range(args.unlearning_epochs):
        ...
        for forget_batch, retain_batch in zip(forget_train_dataloader, retain_train_dataloader):
            ...
            loss = compute_combined_loss(model, forget_batch, criterion, args, device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--top_data', type=float, default=1)  
