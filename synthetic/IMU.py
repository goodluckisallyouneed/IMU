...
def _is_head_param(name):
    if not any(f"layers.{i}." in name for i in HEAD_LAYER_RANGE):
        return False
    return any(name.endswith(suffix) for suffix in HEAD_PARAM_SUFFIXES)


def get_last_params(model):
    return [p for n, p in model.named_parameters() if _is_head_param(n)]


def get_last_named_params(model):
    return [(n, p) for n, p in model.named_parameters() if _is_head_param(n)]
...
class IMU(UnlearnTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        head_param_ids = {id(p) for p in get_last_params(model)}
        ...
        def zero_out_grads():
            for p in model.parameters():
                if id(p) not in head_param_ids and p.grad is not None:
                    p.grad.detach_()
                    p.grad.zero_()
